...
.. code-block:: bash
   ...
   -e NAVI_DEBUG=0 \
   -e NAVI_DEBUG_TOKEN=<debug-token> \
...
+----------------------+----------------------------------------------------------------+------------------------------------------------------+
| NAVI_DEBUG_TOKEN     | Required when NAVI_DEBUG is set to 3. Access token for the     | ``R3pl4ceM3WithAStrongToken``                        |
|                      | /queue, /history and /buffer diagnostics endpoints.             |                                                      |
+----------------------+----------------------------------------------------------------+------------------------------------------------------+
...
#. When level 3 debugging is enabled and ``NAVI_DEBUG_TOKEN`` is set, the following web pages are available from the web service

   * url-to-web-service/queue?token=<NAVI_DEBUG_TOKEN>

     * Shows the tracks in the current queue

   * url-to-web-service/history?token=<NAVI_DEBUG_TOKEN>

     * Shows the tracks that have already been played

   * url-to-web-service/buffer?token=<NAVI_DEBUG_TOKEN>
